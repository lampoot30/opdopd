...
<head>
    <meta charset="UTF-8" />
    <title>Book Appointment for Relative - AMH Hospital</title>
...
<div class="main-content">
    <div class="container-fluid">
        <h1 class="h2 mb-4">Book Appointment for <span class="text-success">${relativeProfile.givenName} ${relativeProfile.surname}</span></h1>
        ...
        <%-- Remove the old success alert banner
            ...
            <div class="card-body p-4">
                <form id="appointmentForm" action="BookAppointmentServlet" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-4" id="patientTypeSelection">
                        <label class="form-label h5">Is this relative a new or old patient of the hospital?</label>
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="oldPatient">Old Patient</label>
                        </div>
                    </div>
                    ...
                    <!-- Patient Information -->
                    <h5 class="mt-4 mb-3">Patient Information (Relative)</h5>
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <label for="lastName" class="form-label">Last Name</label>
                            <input type="text" class="form-control" id="lastName" name="lastName" required value="${relativeProfile.surname}">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="givenName" class="form-label">Given Name</label>
                            <input type="text" class="form-control" id="givenName" name="givenName" required value="${relativeProfile.givenName}">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="middleName" class="form-label">Middle Name</label>
                            <input type="text" class="form-control" id="middleName" name="middleName" value="${relativeProfile.middleName}">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="birthday" class="form-label">Birthday</label>
                            <input type="date" class="form-control" id="birthday" name="birthday" required value="${relativeProfile.dateOfBirth}">
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="gender" name="gender" required value="${relativeProfile.gender}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="religion" class="form-label">Religion</label>
                            <input type="text" class="form-control" id="religion" name="religion" value="${relativeProfile.religion}">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="contactNumber" class="form-label">Contact Number</label>
                        <input type="tel" class="form-control" id="contactNumber" name="contactNumber" required value="${relativeProfile.contactNumber}">
                    </div>
                    <div class="mb-3">
                        <label for="address" class="form-label">Address</label>
                        <textarea class="form-control" id="address" name="address" rows="2" required>${relativeProfile.permanentAddress}</textarea>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    dateInput.addEventListener('blur', function() {
        ...
        const schedule = serviceSelect.options[serviceSelect.selectedIndex].getAttribute('data-schedule') ? serviceSelect.options[serviceSelect.selectedIndex].getAttribute('data-schedule').toLowerCase() : '';
        let isValid = true;
        ...
        if (isValid && schedule.includes('monday to friday') && (dayOfWeek === 0 || dayOfWeek === 6)) isValid = false;
        else if (isValid && schedule.includes('wednesday and thursday') && dayOfWeek !== 3 && dayOfWeek !== 4) isValid = false;
        else if (isValid && schedule.includes('2nd and 4th wednesday') && (dayOfWeek !== 3 || (weekOfMonth !== 2 && weekOfMonth !== 4))) isValid = false;
    ...
    calculateAge(); // Calculate age on page load for pre-filled data
    ...
    // If there is only one service option (e.g., for pediatric patients),
    // auto-select it and disable the dropdown.
    if (serviceSelect.options.length === 2) { // 1 for placeholder, 1 for the actual service
        serviceSelect.selectedIndex = 1;
        serviceSelect.disabled = true;
        serviceSelect.dispatchEvent(new Event('change')); // Trigger change to show schedule info
    }

    // --- New: Handle Success Modal ---
    const urlParams = new URLSearchParams(window.location.search);
    const successMessage = urlParams.get('success');
    const relativeId = urlParams.get('relativeId'); // Keep relativeId for URL cleaning
    ...
    if (successMessage) {
        ...
        const newUrl = `${window.location.pathname}?relativeId=${relativeId}`;
