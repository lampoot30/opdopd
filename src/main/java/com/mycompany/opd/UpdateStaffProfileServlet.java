...
@WebServlet("/update-staff-profile")
@MultipartConfig
public class UpdateStaffProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            conn.commit();
            AuditLogger.log(userId, username, "Staff", "Updated own profile.");
            response.sendRedirect("staff-profile?success=Profile updated successfully.");

        } catch (SQLException e) {
            e.printStackTrace();
            response.sendRedirect("staff-profile?error=Database error: " + e.getMessage());
        }
    }
}